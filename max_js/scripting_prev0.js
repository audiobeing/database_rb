...
function script_match_boolean(){
	...
	uo = {c:1, ex: 1165,ey:500,px:112,py:450, dict:d}; 
	create_umenu(uo);
	// create_umenu_match_boolean_1(); 
	///// create umenu for comparison expressions
	create_umenu_match_boolean_2(); 
...
//	pat.connect(sprintf,0,mess,1); 
//	pat.connect(bang,0,sprintf,0); 
	pat.connect(umenu0,1,join0,0); 
	pat.connect(umenu1,1,join0,1); 
	pat.connect(umenu2,1,join0,2); 
	pat.connect(textedit0, 0,unjoin0,0); 
	pat.connect(unjoin0,1,join0,3); 
	pat.connect(umenu3,1,join0,4); 
	pat.connect(umenu4,1,join0,5); 
	pat.connect(unjoin1,1,join0,6); 
	pat.connect(textedit1,0,unjoin1,0); 
	pat.connect(sprintf,0,join1, 0); 
	pat.connect(join0,0,join1,1); 
	pat.connect(join1,0, zl_filter0, 0); 
	pat.connect(bang0,0,sprintf,0); 
	pat.connect(umenu4,0,bang0,0); 
	pat.connect(zl_filter0,0,script,0); 
//	mess.message('from javacript'); 
counter++; 
...
function create_sprintf_match_boolean0(){
	sprintf = pat.newdefault(1010, 550, "sprintf", "q_match_boolean"); //, "%f","%s", "%s", "%f", "%s","%s","%s"); 
	obj_n(sprintf, 'sprintf0_');  
	
	join0 = pat.newdefault(1158,550,'join',7); 
	obj_n(join0, 'join0_'); 
	
	join1 = pat.newdefault(1010,580, 'join', 5); 
	obj_n(join1, 'join1_'); 
 
	zl_filter0 = pat.newdefault(1010,610, 'zl.filter', 0); 
	obj_n(zl_filter0, 'zl_filter0_'); 
	
	bang0 = pat.newdefault(980,550,'button'); 
	obj_n(bang0,'bang0_'); 
...
// f,k,c
	...
	post(arg_length); 
	var mong = "db.collection('database0').find({'"+arg[arg.length-1]+"':[{'"+arg[5]+"':{'"+arg[4]+"':"+arg[3]+"}},{'"+arg[2]+"':{'"+arg[1]+"':"+arg[0]+"}}]}).toArray(function(err,doc){if(doc != null){for(var i=0; i<doc.length; i++) {var name = doc[i].name;console.log(JSON.stringify(doc[i].name)); maxDict[name] = doc[i]; };}; writeJson();});"
					dict_query.set('mongodb', mong); 
	// remove_objects(); 
}
