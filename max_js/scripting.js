{
	...
	var dict_mongodb = new Dict('mongodb'); // rb
	var dict_buffer = new Dict('db_buffer'); 
	var pat = this.patcher;  //rb
	...
	//
	var mongodb_toArray = ".toArray(\n function(err,doc){\n if(doc != null){\n for(var i=0; i<doc.length; i++) {\n var name = doc[i].session+'_'+doc[i].ges_id;\n console.log(JSON.stringify(doc[i].session));\n maxDict[name] = doc[i]; \n};}; \n writeJson();});"; 
	}
...
function script_match(x,y){
 ...
 //	mess.message('from javacript'); 
	counter=counter+1; 
} // match within array or single value 

function match_session(){
	var np = ""; 
	var d; 
	/// create sprintf to hold arguments
	create_sprintf("q_match_session"); 
	// create text edit for sesssion name
	uo = {c: 0, ex: 800,ey: 480, px:15, py:450}
	create_textedit(uo);
	/// create umenus for keys 
	d = dict_buffer.get('keys'); 
	uo = {c:0, ex: 910,ey:480,px:122,py:450, dict:d}; 
	create_umenu(uo);
	///// create umenu for comparisons
	d=dict_mongodb.get('comparison'); 
	uo = {c:1,ex:1025,ey:480,px:229,py:450, dict:d}; 
	create_umenu(uo); 
	uo = {c: 1, ex: 1140,ey: 480, px:336, py:450}
	create_textedit(uo);


	pat.connect(unjoin_0,1,join_0,0);
	pat.connect(umenu_0,1,join_0,1);
	pat.connect(umenu_1,1,join_0,2);
	pat.connect(unjoin_1,1,join_0,3);
...
function script_match_sessionX(x,y){
	var np = ''; 
	/// create sprintf to hold arguments
	create_sprintf_match_session(); 
	//// create message box for troubleshooting
	create_message();  
	//// create bang for troubleshooting
 	bang = pat.newdefault(1010,500,"button");  
...
function query_constructor(){
	var create = {
		...
		dy: 0,
		dict: {},
		count: {sprintf:0,bang:0,umenu:0,textedit:0},
		sprintf: function(functionCall){
			/// the first argument is the function to be called each after is the input type (%f %s %i)
			var np; 
			var a = arguments; 
			var args = "'"+a[0]+"'"; 
			//var p = {x:this.x+this.dy, y:this.y+this.dy}; 
			for(var i=1; i<arguments.length; i++){
				if(a[i]!== "string"){a[i]= String(a[i])}
				if(i == arguments.length-1){args = args+",'"+ a[i]+"'"; }
				else{args = args+",'"+ a[i]+"'"}
			} 
			sprintf = 'sprintf = pat.newdefault('+p.x+","+ p.y+ ",'sprintf',"+ args+')'; 
			eval(sprintf); 
			// sprintf = pat; 
			post(sprintf);
			np= 'sprintf_'+this.count.sprintf;  
			this.count.sprintf++; 
			sprintf.varname = np; 
			obj_names.push(np); 
			}, 
...
function create_sprintf_match_session(){
	sprintf = this.patcher.newdefault(1010, 550, "sprintf", "q_match", "%f","%s", "%s"); 
	np= 'sprintf'+counter;  
	sprintf.varname = np; 
	obj_names.push(np); 
} // script_match(); 
function create_sprintf_session(){
	sprintf = this.patcher.newdefault(1010, 550, "sprintf", "q_session", "%s","%s", "%s"); 
	np= 'sprintf'+counter;  
	sprintf.varname = np; 
	obj_names.push(np); 
}
...
function q_match_session(){
	...
	function p_arg(arg){
		post(arg.length+'\n'); 
		if(typeof(arg[0]!== "string")){arg[0]="'"+arg[0]+"'"}
	//	post('p_arg called \n'); 
		var a1 = arg[0]; 
		for(var i=1; i<2; i++){
			post("["+i*ac+","+(i*ac+2)+","+(i*ac+1)+"]"); 
			post(arg[i*ac]+" "+arg[i*ac+2]+" "+arg[i*ac+1]+'\n'); 
		...
		//	post(ac+'\n'); 
			...
			values = value_buf; 
			post('values = '+values+'\n'); 
			var mong = "db.collection('database0').find(\n {$and:[{session:"+arg[0]+"}, "+values+"]}\n )"+mongodb_toArray; // { $lt: [ "$qty", 250 ] }
			dict_query.set('mongodb', mong); 	
			// code_view.set(mong); 
			// // code_write.set('"'+mong+'"'); 
			code_view.set('"'+mong+'"'); 
...
function q_session(){
	...
	//	post('p_arg called \n'); 
		for(var i=0; i<1; i++){
			post("["+i*ac+","+(i*ac+1)+","+(i*ac+2)+"]"); 
			post(arg[i*ac]+" "+arg[i*ac+1]+" "+arg[i*ac+2]+'\n'); 
			if(typeof(arg[i*ac])!= "string"){arg[i*ac] = String(arg[i*ac])}
			value_buf = "{'"+arg[i*ac+1]+"':{"+arg[i*ac+2]+": '"+arg[i*ac]+"'}}"
			// post(value_buf); 
			av++; 
			ac = av*3;
