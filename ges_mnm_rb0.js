...
//         	send(ev.data); 
        	...
        	if(c==0) {
        			console.log('error: type not understood. Currently only db, query, and analysis are functional'); 
        			send('error: type not understood. Currently only db, query, and analysis are functional')
        			}
        };
...
function writeJson_update_keys(){
	...
	// dbQuery.json used to be test.json
	  ...
	  console.log('dbQuery.json written by writeJson_update_keys()');		  
			...
			send('read_update_keys dbQuery.json');  
			send('"'+x+'"'); 
